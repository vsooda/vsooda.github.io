...
![image](http://vsooda.github.io/assets/hmmtts/overview1.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/phone_hmm.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/hmm_obervation.jpg =350x)

在单音素hmm模型中，每个状态对应大量的观测值， 需要用个模型对状态到观测进行建模。一般选用多维高斯模型。之所以用多维，是因为观测值是多维的。
观测模型为：

![image](http://vsooda.github.io/assets/hmmtts/eq_observation.jpg =350x)


在训练和合成的时候:

![image](http://vsooda.github.io/assets/hmmtts/eq_overview.jpg =350x)

具体合成时:

![image](http://vsooda.github.io/assets/hmmtts/eq_sythesis.jpg =350x)


上面只是单个音素的模型，在句子中，通常的做法是将每个词对应的音素模型从左到右连接起来构造HMM网络。下图是a， i两个音素构造的hmm网络，在合成的时候需要决定:每个状态对应的观测，每个音素对应多少个状态（duration模型）。

![image](http://vsooda.github.io/assets/hmmtts/sentence_hmm.jpg =350x)

###HMM模型 Baum-Welch训练
在训练的时候，有的直接将整个词训练称为whole word模型；有的将词划分为音素再训练，则称为sub-word模型，这种训练也称为embeded training。参见[^htkbook] p7


![image](http://vsooda.github.io/assets/hmmtts/isolate_embed_training.png =400x)
...
![image](http://vsooda.github.io/assets/hmmtts/eq_observation1.jpg =350x)


如果每个状态对应的观测值都已经标注好(对齐)， 那么直接可以求得均值和协方差：

![image](http://vsooda.github.io/assets/hmmtts/eq_ob_mean.jpg =250x)
...
![image](http://vsooda.github.io/assets/hmmtts/eq_mean_es.jpg =250x)

HMM前向算法：

![image](http://vsooda.github.io/assets/hmmtts/eq_forward.jpg =250x)

后向算法：

![image](http://vsooda.github.io/assets/hmmtts/eq_backward.jpg =250x)


将第t帧属于第j状态的概率为：

![image](http://vsooda.github.io/assets/hmmtts/eq_state_pro.jpg =250x)
...
![image](http://vsooda.github.io/assets/hmmtts/viterbi_training.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/Viterbi_trainning_topology.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/phone_model_type.jpg =350x)

考虑到词内、词间的不同，三音素模型的划分又有不同。(可能的优化点，与停顿相关)

![image](http://vsooda.github.io/assets/hmmtts/word_boundary_model.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/state_tying.jpg =350x)

下面考虑最简单的绑定：

![image](http://vsooda.github.io/assets/hmmtts/state_tying_simple.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/state_tying_tree.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/eq_duration.jpg =250x)
...
![image](http://vsooda.github.io/assets/hmmtts/hmm_vs_hsmm.jpg =350x)
...
![image](http://vsooda.github.io/assets/hmmtts/state_observation_mean.jpg =350x)

![image](http://vsooda.github.io/assets/hmmtts/state_observation_too_smooth.jpg =350x)

![image](http://vsooda.github.io/assets/hmmtts/state_observation_smooth.jpg =350x)
