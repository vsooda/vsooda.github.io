...
categories: code
---
CMake 为“cross platform make”的缩写。
...
EXE = test_iris test_esr_shape test_pupil test_track
all: test_iris test_esr_shape test_pupil test_track
test_iris :  test_iris.cpp $(OBJ)
	$(CC) -o $@ $@.cpp  $(OBJ) $(LIB) $(INC) $(CFLAG)
	mv $@ ../bin/
test_pupil :  test_pupil.cpp $(OBJ)
	$(CC) -o $@ $@.cpp  $(OBJ) $(LIB) $(INC) $(CFLAG)
	mv $@ ../bin/
test_esr_shape : test_esr_shape.cpp $(OBJ)
	$(CC) -o $@ $@.cpp   $(OBJ) $(LIB) $(INC) $(CFLAG)
	mv $@ ../bin/

test_track : test_track.cpp $(OBJ)
	$(CC) -o $@ $@.cpp   $(OBJ) $(LIB) $(INC) $(CFLAG)
	mv $@ ../bin/
...
Makefile的问题在于不好跨平台。而且编写复杂。cmake能够自动生成makefile文件。
...
2. 使用set设置变量。父目录设置的变量在子目录也可以直接使用。
3. 使用add_executable来生成可执行文件，使用add_library来生成库，选项STATIC生成静态库，选项SHARED生成动态库。紧接着生成目标，使用target_link_libraries来设置依赖库。
4. 使用ADD_SUBDIRECTORY包含子目录。
5. 使用ADD_DEFINITIONS用来设置程序内部宏。
6. INCLUDE_DIRECTORIES头文件搜索路径。LINK_DIRECTORIES库文件搜索路径。
7. 使用install来发布。
8. 使用MESSAGE输出调试信息
